package algorithm.hittingSet.DynHS1;
...
public class DynHS1 {
    ...
    List<DynHSNode1> coverNodes;


    public DynHS1(int nEle) {
        nElements = nEle;
    ...
    public void initiate(List<Long> edges) {
        ...
        coverNodes = walkDown(new DynHSNode1(nElements, elementsMask, minEdges));
    }


    List<DynHSNode1> walkDown(DynHSNode1 root) {
        Set<Long> walked = new HashSet<>();
        List<DynHSNode1> newCoverNodes = new ArrayList<>();
    ...
    List<DynHSNode1> walkDown(List<DynHSNode1> oldCoverNodes) {
        Set<Long> walked = new HashSet<>();
        List<DynHSNode1> newCoverNodes = new ArrayList<>();

        for (DynHSNode1 oldNode : oldCoverNodes)
            walkDown(oldNode, newCoverNodes, walked);

        return newCoverNodes;
    }

    void walkDown(DynHSNode1 nd, List<DynHSNode1> newNodes, Set<Long> walked) {
        ...
        while (addCandidates > 0) {
            if ((addCandidates & 1) != 0L) {
                DynHSNode1 childNode = nd.getChildNode(e, childCand);
    ...
    public void insertEdges(List<Long> insertedEdges) {
        ...
        List<DynHSNode1> coverNodes1 = new ArrayList<>();
        for (DynHSNode1 prevNode : coverNodes) {
    ...
    List<DynHSNode1> removeVerticesFromNodes(List<Long> minRmvdEdges, Set<Long> removedEdges) {
        long affected = 0;
    ...
    public List<Long> getSortedMinCoverSets() {
        return hasEmptyEdge ? new ArrayList<>() : coverNodes.stream()
                .map(DynHSNode1::getElements)
                .sorted()
                .collect(Collectors.toList());
    }

    public List<Long> getMinCoverSets() {
        return hasEmptyEdge ? new ArrayList<>() : coverNodes.stream().map(DynHSNode1::getElements).collect(Collectors.toList());
    }
